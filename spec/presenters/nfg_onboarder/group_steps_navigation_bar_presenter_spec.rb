...
describe NfgOnboarder::GroupStepsNavigationBarPresenter do
  ...
  describe "#href(step, path: '')" do
    ...
    context 'when #before_last_visited_point_of_no_return? is false' do
      ...
      context 'when onboarding has some progress' do
        let(:controller) { 'Fake::TestOnboarder' }
        let(:onboarding_path) { "/onboarding/test_onboarder/#{high_level_first_step}/#{tested_first_step}" }
        let(:progress) { { high_level_first_step.to_s => [tested_first_step] } }

        before do
          allow(h.controller.class).to receive(:name).and_return(controller)
        end
